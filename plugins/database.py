...
class Database:
    ...
    async def add_user(self, id, name):
        user = dict(id=id, name=name, b_name=None, c_link=None)
    ...
    # ---- Visit/Play Count Management Methods (ASYNC) ----
    
    async def record_visit(self, user_id: int):
        await self.plays_col.update_one(
            {'user': user_id},
            {'$inc': {'count': 1}, '$setOnInsert': {'withdraw': False}},
            upsert=True
        )
    ...
    async def reset_count(self, user_id: int):
        await self.plays_col.update_one({'user': user_id}, {'$set': {'count': 0}})

    # ---- Withdrawal Status Management Methods (ASYNC) ----
